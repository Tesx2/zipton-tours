...
async function fetchJson(url) {
  ...
  const jsonResponseFromWordPress = await request(`${url}${separator}i=1`, {
...
exports.handler = async () => {
  try {
    ...
    const items = await fetchJson(`${WORDPRESS_BASE_URL}/social-feed?${params.toString()}`);
    return jsonResponse(200, items);
  } catch (error) {
    return jsonResponse(500, {
      message: "Failed to load Social Feed items.",
      error: error.message
    });
  }
};
